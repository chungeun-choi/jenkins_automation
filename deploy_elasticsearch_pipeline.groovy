pipeline {
    agent any 
    environment {
        PATH_SINGLE = "docker/single"
        PATH_MASTER_SAVLE = "docker/master-slave"
        PATH_RAFT_CLUSTER = "docker/raft-cluster"
    }
    stages {
        ...
        stage("Deploy elasticsearch cluster") {
            steps {
                script {
                    if (struct_value == "single") {
                        sh (script: "python3 deploy.py $PATH_SINGLE")
                    }
                    if (struct_value == "master-slave") {
                        sh (script: "python3 deploy.py $PATH_MASTER_SAVLE")
                    }
                    if (struct_value == "raft-cluster") {
                        sh (script: "python3 deploy.py $PATH_RAFT_CLUSTER")
                    }
                }
            }
        }
